...
// taoBang(DSNDTimKiem.MangNguoiDung)
$('.tdHoTen').each(function(){
    var hoTen = $(this).text();//Lay ra ho ten
    var viTriTuKhoa = hoTen.search(tuKhoa);
    var chuoiKetQua = `${hoTen.substring(0,viTriTuKhoa)}
     <span class="InDam">${tuKhoa}</span>
     ${hoTen.sunstring(viTriTuKhoa)}`
    $(this).html(chuoiKetQua);//gan lai cho html
})
});
//Dinh nghia 1 plugin trong jquery
window.jQuery.prototype.NhapNhay = function(soLan){
    var selecter = $(this);
    for(var i = 0; i<soLan; i++){
        selecter.fadeIn(300);
        selecter.fadeOut(300);
    }
}
// $('#txtTuKhoa').click(TimKiemNguoiDung);
